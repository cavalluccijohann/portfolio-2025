...
const suggestions = [
  { id: 1, label: 'Raycast & Johann', value: 'What is the connection between Raycast and Johann?' },
  { id: 2, label: 'Where is Johann from?', value: 'Where is Johann from?' },
  { id: 3, label: 'Johann\'s latest project?', value: 'What is Johann\'s latest project?' },
  { id: 4, label: 'Johann\'s skills?', value: 'What are Johann\'s skills?' },
  { id: 5, label: 'Johann\'s experience?', value: 'What is Johann\'s experience?' },
]
...
const draftCharCount = computed(() => input.value.length)
...
<template>
  <USidebar
    ...
    :menu="{ ui: { content: 'max-w-2xl side-chat-palette' } }"
    :ui="{
  ...
  >
    ...
    <template #default>
      <div class="bg-gradient-to-b from-white to-white/0 dark:from-black dark:to-black/0 absolute h-10 w-full z-10" />
      ...
      <div class="flex h-full min-h-0 flex-col gap-3">
        ...
        >
          ...
          <div class="w-full">
            ...
            <CarrouselSuggestions :suggestions @select="sendSuggestion" />
          </div>
        </div>
      ...
      <div class="bottom-30 bg-gradient-to-t from-white to-white/0 dark:from-black dark:to-black/0 absolute h-5 w-full" />
    </template>
...
.side-chat-assistant :deep(.comark-content > :first-child) { margin-top: 0 !important; }
.side-chat-assistant :deep(.comark-content > :last-child) { margin-bottom: 0 !important; }
.side-chat-assistant :deep(.comark-content li) { margin-top: 0; margin-bottom: 0.125rem; }
.side-chat-assistant :deep(.comark-content li > p) { margin: 0; }
